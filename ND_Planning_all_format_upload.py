...
def load_and_preprocess_data(file_uploader_object):
    ...
    required_columns = ["Order ID", "LAT", "LON", "SLA STS", "Trip ID", "Time Check", "SLA"]
    # Add 'DP Time' to required_columns if it's always expected, or handle its absence.
    # For now, let's assume it's optional and add it if present.
    # If it must be present, uncomment the line below:
    # required_columns.append("DP Time")
    ...
    # Ensure 'DP Time' column exists, if not, create it as empty string to avoid KeyError later
    if 'DP Time' not in merged_df_processed.columns:
        merged_df_processed['DP Time'] = ''

    # ใช้ฟังก์ชัน parse_time_check
    merged_df_processed["Time Check Status"] = merged_df_processed["Time Check"].apply(parse_time_check)
    merged_df_processed["Trip ID"] = merged_df_processed["Trip ID"].astype(str).str.strip()
...
if processed_data_tuple is not None:
    ...
    # Filter by Trip ID
    rider_options = sorted(filtered_df["Trip ID"].unique())
    selected_riders = st.sidebar.multiselect("เลือก Trip ID:", rider_options)
    if selected_riders:
        filtered_df = filtered_df[filtered_df["Trip ID"].isin(selected_riders)]
    ...
    if not filtered_df.empty:
        ...
        if len(destination_coords) > 20:
            st.warning("⚠️ มีจุดหมายปลายทางมากกว่า 20 จุด จะแสดงเส้นทางสำหรับ 20 จุดแรกเท่านั้น")
            destination_coords = destination_coords[:20]
    ...
    folium.Circle(location=[center_lat, center_lon], radius=10000, color='blue', opacity=0.4, fill=True, fill_color='red', fill_opacity=0.05, popup="รัศมี 10 กม.").add_to(m)
...
# จัดกลุ่มข้อมูลสำหรับแสดงผลบนหมุด
    grouped = filtered_df.groupby(["LAT", "LON"]).agg(
        ...
        Trip_ID=('Trip ID', lambda x: ", ".join(sorted(set(map(str, x))))),
        time_checks=('Time Check', lambda x: ", ".join(sorted(filter(None, x)))),
    ...
    for idx, row in grouped.iterrows():
        ...
        <b>Time Check:</b> {row['time_checks'] if row['time_checks'] else 'N/A'}<br>
    ...
    # --- แสดงตารางข้อมูล ---
    columns_to_display = ["Order ID", "SLA", "Trip ID", "SLA STS", "Time Check", "Time Check Status", "Zone"]
